...
function getLibraryInfo(libraryName) {
	var urlString = "https://api.kirjastot.fi/v3/organisation?with=schedules&period.start=today&period.end=today&name=" + libraryName;
...
function showLibraryInfo(jsonText) {
	...
	var outputText = libraryObject.items[0].name.en + "<br />" + libraryObject.items[0].address.street.en + "<br />" + libraryObject.items[0].address.zipcode + " " + libraryObject.items[0].address.city.en + "<br /> " + 
	
	'<a href="' + libraryObject.items[0].homepage.en + '">' + libraryObject.items[0].homepage.en + '</a><br>';
	...
	if (libraryObject.items[0].schedules[0].closed === true) {
		...
		outputText += "Opening times today: " + libraryObject.items[0].schedules[0].opens + " - " + libraryObject.items[0].schedules[0].closes;
	}
